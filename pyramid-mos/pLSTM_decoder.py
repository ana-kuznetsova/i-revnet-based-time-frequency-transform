...
class Attention(nn.Module):
	def __init__(self):
		...
		def forward(self, query, key, value, lens, device):
			'''
...
class Decoder(nn.Module):
	def __init__(self, vocab_size, hidden_dim, value_size=128, key_size=128,  isAttended=True):
		...
		self.mos = nn.Linear(key_size+value_size,vocab_size)

	def forward(self, key, values, lens, device, text=None, train=True):
		'''
		...
		if(train):
			...
			text = torch.transpose(text, 1, -1).long()
